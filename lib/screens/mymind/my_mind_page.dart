...
class _ResultPanelState extends State<_ResultPanel> {
  ...
  final List<ResultAccount> _items = [];
    ...
    return DecoratedBox(
      ...
      child: Row(
        children: [
          ...
          _filterButton(
              '\ud604\uc2e4 \ud504\ub85c\ud30c\uc77c', _ResultFilter.reality),
          _filterButton(
              '\uc774\uc0c1 \ud504\ub85c\ud30c\uc77c', _ResultFilter.ideal),
        ],
      ),
    );
  }
...
class _ResultCard extends StatelessWidget {
  ...
  String _tester(ResultAccount item) {
    final name = item.result?['TEST_TARGET_NAME'] ?? '';

    return name is String && name.isNotEmpty ? name : '미지정';
  }

  String? _resultType(ResultAccount item, {String key = 'DESCRIPTION'}) {
